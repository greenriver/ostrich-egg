...
SqlExpressionObject = namedtuple(typename='SqlExpressionObject', field_names=['select', 'group_by', 'aliases'])

RedactionIterationResult = namedtuple(
    typename='RedactionIterationResult', field_names=['other_dimension_values', 'remapped_lookup']
)
...
class Engine:
    ...
    def __init__(
        ...
        self.connector = Connector(table_name=self.active_dataset.name, **config.datasource.connection_params)
    ...
    def metric_aliases(self) -> dict:
        ...
        """
        metric_aliases = {metric.alias: metric.render_as_sql_expression() for metric in self.metrics}
        return metric_aliases

    @property
    def metric_sql_list(self) -> list:
        return [f"{metric} as {identifier(alias)}" for alias, metric in self.metric_aliases.items()]
    ...
    def make_updated_expressions(self) -> dict:
        ...
        for dimension, redactions in self.redactions.items():
            ...
            for redaction in redactions:
                whens.extend(self.make_when_statements_from_redaction(dimension=dimension, redaction=redaction))
            updated_expressions[
                dimension
            ] = f''' \
            case
    ...
    def dimensions_as_sql_expressions(self, dimensions: list) -> SqlExpressionObject:
        ...
        sql_object = SqlExpressionObject(
            ...
            group_by=', '.join(
                [f'{updated_expressions.get(dimension, identifier(dimension))}' for dimension in dimensions]
            ),
    ...
    def dimension_with_replacements(dimension: str, replacements: dict) -> str:
        ...
        """
        dimension_replacements = '\n '.join([f"when '{k}' then '{v}'" for k, v in replacements.items()])
        case_statement = f"""
        case "{dimension}"
            {dimension_replacements}
            else "{dimension}"
    ...
    @staticmethod
    def make_when_statements_from_redaction(redaction: RedactionIterationResult, dimension: str) -> List[str]:
        whens = []
        for old, new in redaction.remapped_lookup.items():
            redaction.other_dimension_values[dimension] = old
            whens.append(make_when_statement_from_dict(data=redaction.other_dimension_values, value=new))
        return whens

    def get_wrapper_metrics_pass_expression_from_threshold(self) -> str:
        """
        Construct the expression that determines if the row itself passes privacy thresholds
        It compares each metric to the threshold and returns a single boolean for whether the row in question passes.

        A possible question to address is should each metric have its own threshold?
        How realistically will we use metrics that aren't "count distinct people"?
        ...
        {%- endfor %}
        '''
        context = {"metric_aliases": self.metric_aliases, "allow_zeroes": self.allow_zeros, "threshold": self.threshold}
    ...
    def get_rendered_aggregation_query(self, dimensions: list = None, table_name: str = None) -> str:
        dimensions = dimensions or self.active_dimensions
        dimensions_as_sql_object = self.dimensions_as_sql_expressions(dimensions=dimensions)
        context = {
            "metric_list": self.metric_sql_list,
            "dimensions": dimensions_as_sql_object,
            "table_name": table_name or self.connector.table_name,
            "wrapper_list_check": self.get_wrapper_metrics_pass_expression_from_threshold(),
            "is_anonymous": IS_ANONYMOUS_COLUMN,
        }

        sql_template = """
        select *, {{wrapper_list_check}} as {{is_anonymous}}
        from (
    ...
    def run_aggregation(self, dimensions: list = None, result_name=DEFAULT_RESULT_NAME, table_name: str = None):
        """
        ...
        """
        dimensions = dimensions or self.active_dimensions
        sql = self.get_rendered_aggregation_query(dimensions=dimensions, table_name=table_name)
    ...
    def drop_dimension(self, dimension: str):
        if dimension not in self.active_dimensions:
            raise ValueError(f"Cannot remove dimension {dimension} as it is not presently in active dimensions.")
    ...
    def get_dimension_values_to_redact_with_latency_check(
        ...
        dimensions_to_hold_constant = [d for d in self.active_dimensions if d != dimension]
        sql_expressions = self.dimensions_as_sql_expressions(dimensions=dimensions_to_hold_constant)
        dimension_value_count_sql = f"""
        select dense_rank() over(order by {sql_expressions.aliases}) as peer_id , *
        from (
            select {sql_expressions.select}, {identifier(dimension)} as dimension_value, sum({metric_name}) as metric_value
            from result
        ...
        for peer_id in peer_ids:
            ...
            peer_value_sql = f'''
                select x as peer
                from (
                    select {sql_expressions.aliases}, sum(metric_value) as total
                    from identify_peers
            ...
            this_peer_relation = (
                identify_peers.filter(filter_by_peer).select('dimension_value', 'metric_value').order("metric_value")
            )
            peer_result, must_anonymize_next = self._collect_redactions_from_peer_relation(
    ...
    def _collect_redactions_from_peer_relation(
        ...
        Result = namedtuple('Result', field_names=['dimension_value', 'metric_value'])
        for within_peer_index, local_result in enumerate(map(Result._make, this_peer_relation.fetchall())):
            values_is_fine = local_result.metric_value >= self.threshold
            first_value_is_good = within_peer_index == 0 and values_is_fine
            if not must_anonymize_next and first_value_is_good:
            ...
            elif must_anonymize_next and values_is_fine:
                if working_total < self.threshold:
                    values_to_mask.append(local_result.dimension_value)
                must_anonymize_next = False
                anonymized = True
                break

            elif local_result.metric_value < self.threshold:
                """
                Base case small-cell for suppression.
                """
                logger.debug(
                    f"Redacting {local_result.dimension_value} as it is below the threshold of {self.threshold}."
                )
                values_to_mask.append(local_result.dimension_value)
                must_anonymize_next = True
                working_total += local_result.metric_value

            elif (
                len(values_to_mask) >= 1
                and (not anonymized or must_anonymize_next)
                and local_result.metric_value >= self.threshold
                and row_count > 2
                and working_total >= self.threshold
            ):
                """
                There has been suppression of previous values and we are now safely anonymized
                """
                filter_by_excluded_values = duckdb.ColumnExpression("dimension_value").isnotin(
                    *[duckdb.ConstantExpression(v) for v in values_to_mask]
                )
                remainder, *_ = this_peer_relation.filter(filter_by_excluded_values).sum('metric_value').fetchone()
                ...
                """
                logger.debug(
                    f"Redacting {local_result.dimension_value} as it would latently reveal an unacceptable metric value through subtraction."
                )
                values_to_mask.append(local_result.dimension_value)
                working_total += local_result.metric_value

            elif must_anonymize_next:
        ...
        peer_result = (
            [
                RedactionIterationResult(
                    ...
                    remapped_lookup={dimension_value: masking_value for dimension_value in values_to_mask},
                )
            ]
            if values_to_mask
            else []
    ...
    def update_the_dataset(self, dimension, existing_values: list, new_value: str = 'Redacted'):
        sql = f'''
        ...
        for param in parameters:
            redactions.extend(
                self.get_dimension_values_to_redact_with_latency_check(
                    dimension=param.dimension, masking_value=param.merged_value, metric_name=self.metrics[0].alias
                )
            )
    ...
    def replace_with_redacted(self, params: ReplaceWithRedactedParameters):
        ...
        redactions = self.get_dimension_values_to_redact_with_latency_check(
            dimension=params.redacted_dimension, masking_value=params.masking_value, metric_name=self.metrics[0].alias
        )

        self.redactions[params.redacted_dimension] = self.redactions.get(params.redacted_dimension, [])
        self.redactions[params.redacted_dimension].extend(redactions)
        self.run_aggregation()
        self.connector.duckdb_connection.execute('create or replace table output as select * from result')
    ...
    def mark_redacted(self, params: MarkRedactedParameters):
        ...
        if params.non_summable_dimensions:
            ...
            pages = result_table.select(*columns).distinct().to_df().to_dict(orient='records')
            for page in pages:
                ...
                page_redactions = self.get_dimension_values_to_redact_with_latency_check(
                    dimension=dimension_to_use, metric_name=list(self.metric_aliases.keys())[0]
                )
        ...
        self.connector.duckdb_connection.execute('update output set is_redacted = true where not is_anonymous')
        for redaction in redactions:
            for old, new in redaction.remapped_lookup.items():
                redaction.other_dimension_values[dimension_to_use] = old
                filters = dict_to_filter_expressions(data=redaction.other_dimension_values)
                condition = merge_conditions(filters)
                update_sql = f'''
                update output
    ...
    def make_anonymized_dataset(self):
        ...
        if 'output' not in tables:
            self.run_aggregation(
                dimensions=self.active_dimensions, result_name='output', table_name=self.final_source_table
            )
    ...
    def write_anonymized_dataset_to_file(self, file_path=None, **file_kwargs):
        ...
        if not file_path:
            raise ValueError("You must specify a file path at run time or in the config to produce an output file.")
    ...
    def run_one_dataset(self, index: int, dataset: DatasetConfig, output_file: str = None):
        ...
        if dataset.source_file and dataset.source_file in self.source_file_to_table_lkp and self.cache_tables_in_memory:
            ...
            logger.info(f"This process has a table {table_name} in memory for {dataset.source_file}")
        elif dataset.source_file and dataset.source_file not in self.source_file_to_table_lkp:
        ...
        if output_file:
            ...
            if self.cache_tables_in_memory:
                ...
                self.connector.duckdb_connection.execute(f""" drop table if exists "{dataset.name}" cascade """)
            self.connector.duckdb_connection.unregister('output')
            self.connector.duckdb_connection.execute(""" drop table if exists output """)
